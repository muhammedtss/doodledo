...
interface TaskItemProps {
  ...
  index: number;
  isDarkMode: boolean;
  penColor?: string; // string oldu (default, red, gold)
  sticker?: string;  // none, cat, fire
}

export const TaskItem: React.FC<TaskItemProps> = ({ task, onPress, onLongPress, onToggle, index, isDarkMode, penColor, sticker }) => {
  const randomStyles = useMemo(() => {
    const rotation = getRandomRotation();
    const randomWidth = `${85 + Math.floor(Math.random() * 10)}%`;
    // Sticker için rastgele konum
...
const styles = StyleSheet.create({
  card: {
    ...
    overflow: 'visible' // Sticker taşabilir
  },
  pin: {
